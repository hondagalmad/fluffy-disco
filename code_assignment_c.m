function code_assignment_c(input_source, output_filename)
    % Code assignment compressor based on Lecture 7:
    % count symbol occurrences, then assign shorter prefix-free codes
    % to more probable symbols using Shannon-Fano partitioning.

    if nargin < 2
        error('code_assignment_c requires input_source and output_filename.');
    end
    ...
    frequencies = zeros(1, numel(symbols), 'uint32');
...
function save_compressed(filename, encoded_data, symbols, frequencies)
    ...
    fwrite(fid, numel(symbols), 'uint32');
